...
\noindent
with the \ac{MFP} $\uplambda$, the initial \ac{MFP} $\uplambda_0$, the damage constant $\upkappa$ and the fluence $\upphi$ \cite{felix}. Since the measurable quantity is the \ac{CCD} we make the assumption that the \ac{MFP} of electrons and holes are the same which was studied in \cite{felix}. Due to that one can find a relation of the \ac{CCD} to the \ac{MFP} .\par
